...
void circleDectection(SDL_Surface *img, int *resx, int *resy)
{
    ...
    int R = 127; //this is the radius of an average face in pixel,
...
void circleDectection_dynamic(SDL_Surface *img, int *resx, int *resy)
{
    //new matrix to fill the vote count.
    Matrix *matrix = new_matrix(img->w, img->h);
    //make a list of matrices, then for each have a certain R
    //value in the theta loop
    Uint8 r,g,b;

    int R = 50; //this is the radius of an average face in pixel,
    //might need to change this
    int counterx =0;
    int countery =0; //to know if we are in a circle already drawnm
    //so as not to take it
    int i =0;
    int j=0;
    for (i = R+3; i < img->w - R -3; i++)
    {
        ...
        {
            Uint32 pix = get_pixel(img, i, j); //from pixel.c
            //to be able to easily manipulate pixels
            SDL_GetRGB(pix,img->format, &r ,&g, &b);
            if (r == 255) //if it is white
                // face edge pixel (it is in binary since BW)
            {
                        ...
                        (countery >= j+R -R/10|| countery <= j-R + R/10))
                {
                    ...
                    for (size_t theta = 0; theta < 360; theta+=2)
                    {
                        int x = i + R*cos(theta); //cos takes double,
                        //check for this
                        int y = j + R*sin(theta); //sin takes double as well
                        matrix->data[x][y] +=1;
                    }
