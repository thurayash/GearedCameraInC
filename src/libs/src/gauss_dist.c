...
{
    /*
        here we need to measure how many pixels is in an average head on
        each distance of every Circle detection radius
    */

    //int list[] = {(radius of detection, length in pixel of a face) 
    //* number of how many circle radii};
    int xres = abs(x1 - x2);
    int yres = abs(y1 - y2);
    int pixdist = fmax(xres, yres);
    /*
        25 cm is average face (check value!!!)
        list[id][1] == the relative length of the face infered from the
        distance of circle detection
...
/*
    ...
    both dist1 and 2 are inferred from the Circle detection algo
    (which colored rectangle)
*/
